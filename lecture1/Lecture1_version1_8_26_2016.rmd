...
* Select (drag yoour mouse over) 5 or more lines of text (from the History tab), click on "To Source" to send the command to the R Console
...
**Note**: Command completion can beturned on/of through "Tools" -> "Global Options ..." -> "Code" -> "Completeion"
...
* <b>Ctrl-2</b> Moves cursor to the Consule window
* <b>Ctrl-4</b> Moves cursor to the Consule window
...
* `summary()`   shows a sumary of data object 
...
### R Basic Data Types
Now that are familar with R, we are going to look at the variable type that can be created in R
<br>
Be mindful when creating vari
#### (1) Vectors of length one (atomic data types)
...
Used R's c()function to create a vector with the following 24
sets of values:
...
Use: `?c` to determine how the 24 values should be formated as arguments to `c()`.  
...
* Document your script file by adding addtings to explain your work.
* Save your history in a file and store in in your lecture_1 folder
* Exit from RStudio
* Upload your script file and its output and to blackboack or mail it to your instructor with a copy to wampeh@frb.gov and andrew.cohen@frb.gov
